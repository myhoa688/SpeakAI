const fs = require('fs');
let content = fs.readFileSync('frontend/src/pages/InterviewResultPage.tsx', 'utf8');
const startIdx = content.indexOf('const currentScoreColor =');
if (startIdx !== -1) {
  const replacement = `const currentScoreColor = getScoreColorClass(currentAnswer.score);
    ...
    <>
      ...
      {showRating && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: 'var(--bg-card, #1e293b)', borderRadius: '16px', padding: '2rem', width: '340px', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.5)' }}>
            {ratingDone ? (
              <>
                <div style={{ fontSize: '2.5rem', marginBottom: '0.5rem' }}>🎉</div>
                <p style={{ fontWeight: 600, color: '#fff' }}>Cảm ơn bạn đã đánh giá!</p>
              </>
            ) : (
              <>
                <div style={{ fontSize: '1.5rem', marginBottom: '0.25rem' }}>⭐</div>
                <h3 style={{ color: '#fff', marginBottom: '0.5rem' }}>Bạn thấy phiên này thế nào?</h3>
                <p style={{ color: '#94a3b8', fontSize: '0.875rem', marginBottom: '1.5rem' }}>Đánh giá giúp chúng tôi cải thiện trải nghiệm</p>
                <div style={{ display: 'flex', justifyContent: 'center', gap: '0.5rem', marginBottom: '1.5rem' }}>
                  {[1, 2, 3, 4, 5].map(star => (
                    <button key={star} onClick={() => setUserRating(star)}
                      style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '2rem', color: star <= userRating ? '#f59e0b' : '#475569', transition: 'color 0.15s' }}>
                      ★
                    </button>
                  ))}
                </div>
                <div style={{ display: 'flex', gap: '0.75rem' }}>
                  <button onClick={() => setShowRating(false)}
                    style={{ flex: 1, padding: '0.6rem', borderRadius: '8px', border: '1px solid #334155', background: 'transparent', color: '#94a3b8', cursor: 'pointer' }}>
                    Bỏ qua
                  </button>
                  <button disabled={userRating === 0} onClick={async () => {
                    await api.post(\`/interviews/\${id}/rating\`, { rating: userRating });
                    setRatingDone(true);
                    setTimeout(() => setShowRating(false), 1500);
                  }}
                    style={{ flex: 1, padding: '0.6rem', borderRadius: '8px', border: 'none', background: userRating ? '#6366f1' : '#334155', color: '#fff', cursor: userRating ? 'pointer' : 'not-allowed', fontWeight: 600 }}>
                    Gửi đánh giá
                  </button>
                </div>
              </>
  ...
  fs.writeFileSync('frontend/src/pages/InterviewResultPage.tsx', content);
  console.log('Fixed');
}
